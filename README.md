...
- 配置导入 / 导出：标题栏右侧「导出配置」「导入配置」按钮可导出或导入完整配置（含机器清单与界面设置）；导入时自动备份当前配置，可选「覆盖」或「合并追加」
- SSH 私钥路径等低频配置移至标题栏右侧「高级设置」按钮，主界面不再常驻该输入框

## 快速开始

### 方式一：直接使用安装包

下载 `QTX-RemoteLock-v1.0.22.exe`，双击即运行，无需安装 Python。
