...
远程锁屏 / 解锁管理工具（Windows）。图形界面批量管理多台 AIGC 机器，
...
下载 `QTX-RemoteLock-v1.0.2.exe`，双击即运行，无需安装 Python。
